...
import { Plus, KeyRound, Copy, CheckCircle2, AlertTriangle, Trash2, Clock } from 'lucide-react';
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-100">
                {keys.map((k) => (
                  <tr key={k.keyId} className="table-row-hover">
                    <td className="px-6 py-4 text-sm font-medium text-slate-900">{k.label || '—'}</td>
                    <td className="px-6 py-4">
                      <span className="font-mono text-sm text-slate-600">{k.maskedKey}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`badge ${k.environment === 'LIVE' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
                        {k.environment}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600">{k.keyType}</td>
                    <td className="px-6 py-4">
                      <span className={`badge ${k.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                        {k.status}
                      </span>
                    </td>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={copySecret} className="btn-secondary flex-shrink-0">
            ...
            <div className="flex justify-end pt-2">
              <button onClick={() => { setNewKey(null); setCopied(false); }} className="btn-primary">
                Done
              </button>
            </div>
          </div>
